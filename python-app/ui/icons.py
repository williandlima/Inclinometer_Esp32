...
uma cor só, no mesmo espírito do ícone de engrenagem original."""
...
def _arrowhead(tip: QPointF, dir_x: float, dir_y: float, head_len: float) -> QPolygonF:
    """Triângulo de seta na ponta `tip`, apontando em (dir_x, dir_y) (vetor
    unitário). Usado em vez de uma linha com round-cap simples porque um
    round-cap só um pouco maior que a espessura do traço não lê como seta
    (era exatamente esse o problema na primeira versão deste módulo)."""
...
def _arc_arrowhead(cx: float, cy: float, r: float, end_angle_deg: float, clockwise_screen: bool, head_len: float) -> QPolygonF:
    """Seta na ponta final de um arco desenhado com `drawArc` (mesma
    convenção de ângulo: 0°=direita, 90°=cima). `clockwise_screen` indica o
    sentido de desenho do arco tal como aparece na tela (não o sinal do
    `spanAngle` do Qt, que já inverte por causa do eixo Y para baixo) — a
    tangente é estimada por diferença finita em vez de calculada
    analiticamente, pra não repetir o erro de sinal que a primeira versão
    deste ícone tinha."""
    eps = 3.0
...
def gear_icon(color: str, size: int = 22) -> QIcon:
    """Engrenagem — usada no botão de Configurações."""
...
def tilt_icon(color: str, size: int = 20) -> QIcon:
    """Inclinação: horizonte + seta inclinada, ao lado do título do cartão de tilt."""
    pixmap, painter = _new_pixmap(size)
    pen_color = QColor(color)

    # Linha de base (horizonte).
    painter.setPen(Qt.NoPen)
    painter.setBrush(pen_color)
    painter.drawRoundedRect(QRectF(size * 0.06, size * 0.80, size * 0.88, size * 0.09), 1.5, 1.5)

    # Seta inclinada saindo da base.
    angle_deg = -38
...
def pan_icon(color: str, size: int = 20) -> QIcon:
    """Azimute: arco quase completo com seta na ponta, ao lado do título do
    cartão de pan — sugere rotação em torno de um eixo vertical."""
    pixmap, painter = _new_pixmap(size)
    pen_color = QColor(color)
    cx, cy, r = size / 2, size / 2, size * 0.36
    start_deg, span_deg = 15, 260  # sentido anti-horário na tela (span positivo)
...
def target_icon(color: str, size: int = 20) -> QIcon:
    """Mira/alvo — usado no botão Calibrar (define a posição de referência)."""
    pixmap, painter = _new_pixmap(size)
    pen_color = QColor(color)
...
def reset_icon(color: str, size: int = 20) -> QIcon:
    """Seta circular — usado no botão Resetar limites."""
    pixmap, painter = _new_pixmap(size)
    pen_color = QColor(color)
...
def vibration_icon(color: str, size: int = 20) -> QIcon:
    """Onda — usado no botão Modo Vibração."""
    pixmap, painter = _new_pixmap(size)
...
def report_icon(color: str, size: int = 20) -> QIcon:
    """Documento — usado no botão Gerar relatório PDF."""
    pixmap, painter = _new_pixmap(size)
    pen_color = QColor(color)
    path = QPainterPath()
